...
\begin{figure}[htbp]
    ...
    \caption{Example phase portrait of the spruce budworm model showing stable (blue) and unstable (red) equilibria. The green dashed line indicates the current population $x_t$. Where do you think this population will evolve?}
    \label{fig:graph}
\end{figure}
...
Create sliders for:
\begin{itemize}
    \item Growth rate $r$ (range: 0.0 to 1.0, default 0.5).
    \item Carrying capacity $k$ (range: 0.1 to 10.0, default 10.0).
    \item Initial population: set automatically to $k/10$ (the app uses $x_0 = k/10$ by default).
    \item Time slider for evolution (range: 1 to 100, default 10).
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Hysteresis:} Slowly increase the carrying capacity $k$ from 5 to 15. Then slowly decrease it back to 5. Does the population return to the same state? If you are interested in this concept, see section \ref{sec:slow_fast_dynamics}.
...
\subsection{Slow-Fast Dynamics}
\label{sec:slow_fast_dynamics}

The carrying capacity $k$ can be interpreted as a slowly varying parameter in real ecosystems (e.g., due to seasonal changes or forest management). You can simulate this by gradually changing $k$ over time in your app. Changes can be periodic (e.g., sinusoidal) to mimic seasonal effects.
