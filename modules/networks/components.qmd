...
Because we are working on an **undirected** graph, the connection between two nodes does not have a direction. However, we can also create directed graphs where the edges have a direction (from one node to another) using `nx.DiGraph()` instead of `nx.Graph()`. In a directed graph, we would use `add_edge(<source>, <target>)` to specify the direction of the edge.
