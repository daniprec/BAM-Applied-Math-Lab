...
Real networks (social, transportation, communication) are not arbitrary: they tend to have **short paths**, **clustering**, and sometimes **hubs** [@newman2018networks].
...
In the **Gilbert** model $G(N, p)$, we consider every possible pair of nodes and include the edge independently with probability $p$ [@gilbert1959random; @erdos1959random].
...
Watts–Strogatz networks start from a **ring lattice** (high clustering, long paths) and then **rewire edges** with probability $p$ [@watts1998collective].
...
Many real networks exhibit a **heavy-tailed** (sometimes approximately **power-law**) degree distribution: most nodes have small degree, but a few nodes become **hubs** [@barabasi1999emergence; @newman2018networks].

The **Barabási–Albert (BA)** model generates this effect via **preferential attachment** [@barabasi1999emergence]:
