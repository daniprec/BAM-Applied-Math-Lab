...
The SIR (Susceptible–Infected–Recovered) model is the “next step” after SIS: once a node recovers, it becomes **permanently immune**.
...
If a susceptible node has $m$ infected neighbors, we use the same “independent trials” rule as SIS:
...
On a finite graph with $\mu > 0$, the process reaches a time $T$ such that there are **no infected nodes** ($I(T)=0$), and after that nothing can ever change.
...
## Initialize states (node attributes)
