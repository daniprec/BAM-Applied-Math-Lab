...
In this page, we will explore some basic properties of networks, such as **degree**, **density**, **clustering**, and **paths / connected components** [@newman2018networks].
...
Many real networks have **triangles**: if A is connected to B and C, there is a higher-than-random chance that B is also connected to C [@watts1998collective; @newman2018networks].
...
These ideas are quantified by **centrality measures** [@newman2018networks].

### Degree centrality

The simplest centrality is based on degree. In an undirected graph with $N$ nodes, the (normalized) degree centrality is [@newman2018networks]
...
Closeness measures how close a node is (on average) to all others via shortest paths [@sabidussi1966centrality].
...
Betweenness identifies **bridges**: nodes that sit on many shortest paths [@freeman1977betweenness].
Formally,
...
For directed networks (like the web), PageRank is the stationary distribution of a random walk ("random surfer") on the graph [@page1999pagerank].
