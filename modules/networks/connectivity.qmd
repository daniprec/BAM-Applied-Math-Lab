...
These ideas are quantified by **centrality measures**.

### Degree centrality

The simplest centrality is based on degree. In an undirected graph with $N$ nodes, the (normalized) degree centrality is
...
Closeness measures how close a node is (on average) to all others via shortest paths.
...
Betweenness identifies **bridges**: nodes that sit on many shortest paths.
Formally,
...
For directed networks (like the web), PageRank is the stationary distribution of a random walk ("random surfer") on the graph.
