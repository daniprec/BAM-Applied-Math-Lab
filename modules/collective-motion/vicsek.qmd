...
Just with that, we have implemented **steps 1-3**. Now we can compute the new positions based on the new headings, following [@eq-vicsek-xi].
...
You should validate your code. Follow the button below to find experiments that you can run to check if your implementation is correct. You can also compare your results with the original paper and with other implementations online.

[Validate your implementation](vicsek-validation.qmd){.btn .btn-secondary}

Once you are confident in your code, it is time to see the simulation in action! Click on the button below to learn how to animate the Vicsek model and see the particles moving in real time.
