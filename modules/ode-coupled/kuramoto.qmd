...
To visualize the dynamics, imagine points moving on the unit circle. Synchronization corresponds to points clustering around a common phase. @fig-kuramoto-circle shows a snapshot of the oscillators (blue dots) and the order parameter (red arrow) pointing towards the centroid of the phases. If you want to see this in action, run the script in [amlab/odes_coupled/kuramoto.py](../../amlab/odes_coupled/kuramoto.py) to animate the oscillators on the unit circle.

```{python}
#| label: fig-kuramoto-circle
#| fig-cap: 'Oscillators (blue dots) on the unit circle. The order parameter (red arrow) points towards the centroid of the phases. We will learn about the order parameter in the next section.'
#| fig-width: 6
#| fig-height: 6
#| echo: false
