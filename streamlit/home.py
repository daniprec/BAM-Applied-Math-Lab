import streamlit as st

st.markdown("# Applied Maths Lab")
st.markdown(
    "Welcome to the Applied Maths Lab! This is a collection of interactive "
    "demos and simulations for various mathematical models. Use the left tabs "
    "to navigate through the different sessions."
)

st.markdown("---")

# Authors

st.markdown("## Authors")
st.markdown(
    "This project was developed by [Daniel Precioso](https://danielprecioso.com) "
    "and [David Gómez-Ullate](https://www.ie.edu/ieresearchdatalab/news-events/2022/09/david-gomezullate/)."
)


st.markdown("---")

st.markdown("## Resources")
st.markdown(
    "👨‍💻 [GitHub (source code)](https://github.com/daniprec/BAM-Applied-Math-Lab)"
)
st.markdown(
    "Strogatz, S. H. (2018). Nonlinear Dynamics and Chaos: With Applications to Physics, Biology, Chemistry, and Engineering. CRC Press."
)
